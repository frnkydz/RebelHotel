...
public class StudentController {
	...
	void addDateTimeFormatPatterns(Model model) {
		model.addAttribute(
				"student_lastmodified_date_format",
				DateTimeFormat.patternForStyle("SF",
						LocaleContextHolder.getLocale()));
	}

	void addQueryDateTimeFormatPatterns(Model model) {
		model.addAttribute(
				"student_query_date_format",
	...
	public Collection<String> populateDegree() {
		...
		Collection<CatalogRequirement> crs = CatalogRequirement
				.findAllCatalogRequirements();
	...
	public String buildLabelsString(FormStudentQuery formStudentQuery) {
		...
		if (formStudentQuery.getShowUserId()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_userid",
		...
		if (formStudentQuery.getShowEmail()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_email",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowFirstName()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_firstname",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowMiddleName()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_middlename",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowLastName()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_lastname",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowAdmitTerm()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_admitterm",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowGradTerm()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_gradterm",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowCodeOfConductSigned()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_codeofconductsigned",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowLastModified()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_lastmodified",
									null, LocaleContextHolder.getLocale());
		}
		if (formStudentQuery.getShowUserAccount()) {
			properties += ","
					+ messageSource
							.getMessage(
									"label_edu_unlv_cs_rebelhotel_domain_student_useraccount",
									null, LocaleContextHolder.getLocale());
		}
		return properties;
	}
	...
	public String queryList(
			...
			@Valid FormStudentQuery form, BindingResult result, Model model,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		studentQueryValidator.validate(form, result); // rather than assigning
		...
		if (page != null || size != null) {
			...
			model.addAttribute("students", Student.findStudentEntries(
			...
			model.addAttribute(
					...
					(int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1
							: nrOfPages));
			queryResult = studentQueryService.queryStudentsLimited(form,
					sorting, (page.intValue() - 1) * sizeNo, sizeNo);
		} else {
		...
		if (!form.getOutputCsv()
				&& !WebUtils.hasSubmitParameter(request, "downloadExcel")) {
			 ...
			 * since otherwise the pagination that uses queryString would add
			 * those two parameters to the URL each time a different page size
			 * is selected
			 */
			...
			for (int i = 0; i < queryStringArray.length; i++) {
				String check;
				if ((check = queryStringArray[i].split("=")[0]).equals("size")
						|| check.equals("page")) {
					queryStringArray[i] = "";
				}
			}
			...
			// then the contents are placed into the temp file below before sent
			// to the user as a download
			ServletContext servletContext = request.getSession()
					.getServletContext();
			String filename = "student query.csv"; // users will by default
													// receive this file as the
													// name to save as
			String contents = studentQueryService.generateCsv(form, students,
					messageSource); // the contents of the file in a string;
									// alternatively use a byte array
			...
			if (filesize > 0) {
				...
				FileCopyUtils.copy(contents.getBytes(),
	...
	public String create(@Valid FormStudent formStudent, BindingResult result,
		...
		// attempts to find a user account with matching user id first; this may
		// not be desirable
		// likely will want an error message if the found user account is
		// assigned to a different user for some reason
		UserAccount userAccount;
		try {
			userAccount = UserAccount.findUserAccountsByUserId(
					student.getUserId()).getSingleResult();
			userAccount.setEmail(formStudent.getEmail()); // updates email
		} catch (org.springframework.dao.EmptyResultDataAccessException exception) {
			userAccount = UserAccount.fromStudent(student,
					formStudent.getEmail());
		...
		return "redirect:/students/"
				+ encodeUrlPathSegment(student.getId().toString(), request);
	}
	...
	public String createMajor(@PathVariable("id") Long id, Model model) {
		model.addAttribute("formStudentMajor",
				FormStudentMajor.createFromStudent(Student.findStudent(id)));
		return "students/updateMajor";
	}

	@PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_SUPERUSER')")
	@RequestMapping(params = "major", method = RequestMethod.PUT)
	public String updateMajor(@Valid FormStudentMajor fsm,
	...
	public String update(@Valid FormStudent formStudent, BindingResult result,
			Model model, HttpServletRequest request) {
		if (result.hasErrors()) {
			model.addAttribute("formStudent", formStudent);
			addDateTimeFormatPatterns(model);
	...
	public String updateForm(@PathVariable("id") Long id, Model model) {
		model.addAttribute("formStudent",
				FormStudent.createFromStudent(Student.findStudent(id)));
		addDateTimeFormatPatterns(model);
		return "students/update";
	}
	...
	public String findStudentsByFirstNameEquals(
			@RequestParam("firstName") String firstName, Model model) {
		model.addAttribute("students",
				Student.findStudentsByFirstNameEquals(firstName)
						.getResultList());
	...
	public String findStudentsByFirstNameLike(
			@RequestParam("firstName") String firstName, Model model) {
		model.addAttribute("students",
				Student.findStudentsByFirstNameLike(firstName).getResultList());
		return "students/list";
	}
	...
	public String findStudentsByUserAccount(
			@RequestParam("userAccount") UserAccount userAccount, Model model) {
		model.addAttribute("students",
				Student.findStudentsByUserAccount(userAccount).getResultList());
		return "students/list";
	}
